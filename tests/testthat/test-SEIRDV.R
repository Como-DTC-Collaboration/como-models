...
test_that("Transmission parameters can be set and retrieved", {
  ...
  expect_error(
    transmission_parameters(my_model) <- list(
      ...
      nu = c(0.1, 0.2), delta_V = 0.1, delta_R = 0.1, delta_VR = 0.1))
  expect_error(
    transmission_parameters(my_model) <- list(
      beta=1, kappa=0.5, gamma=0.5, mu=0.1,
      nu = 0.1, delta_V = c(0.1, 0), delta_R = 0.1, delta_VR = 0.1))
  expect_error(
    transmission_parameters(my_model) <- list(
      beta=1, kappa=0.5, gamma=0.5, mu=0.1,
      nu = 0.1, delta_V = 0.1, delta_R = c(0.1, 0.2), delta_VR = 0.1))
  expect_error(
    transmission_parameters(my_model) <- list(
      beta=1, kappa=0.5, gamma=0.5, mu=0.1,
      nu = 0.1, delta_V = 0.1, delta_R = 0.1, delta_VR = c(0.1, 0.2)))
})
...
test_that("SEIRDV model runs correctly", {
  ...
  expect_identical(dim(out_df$state), as.integer(c(((10 - 0) / 0.1 + 1) * 7, 4)))
  
  # Check output value for rates equal 0s
  expected_data <- data.frame(
    S = 0.9, E = 0, I = 0.1, R = 0, V = 0, VR=0, Incidence = 0, Deaths = 0)
  ...
  test_data_SEIRV <- out_df_SEIRV[101, 2:7]
  test_data_CD <- out_df_CD[101,2:3]
  row.names(test_data_SEIRV) <- NULL
  row.names(test_data_CD) <- NULL
  expect_identical(test_data_SEIRV, expected_data[1:6])
  expect_identical(test_data_CD, expected_data[7:8])
  ...
  test <- rowSums(out_df_SEIRV[, c(2:7)]) + out_df_CD$Deaths
