...
test_that("can set new transmission parameters for the SEIRAge", {
  ...
  # Test input errors
  expect_error(
    transmission_parameters(my_model) <- list(b=c(1, 0), k=0.5, g=0.5)
  )
  expect_error(
    transmission_parameters(my_model) <- list(b=1, k=c(1, 0), g=0.5)
  )
  expect_error(
    transmission_parameters(my_model) <- list(b=1, k=0.5, g=c(1, 0))
  )
})
