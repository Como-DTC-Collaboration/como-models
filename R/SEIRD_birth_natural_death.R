#' An S4 object representing the SEIRD with natural death and birth.
#'
#' This class represents the SEIRD model taking into account natural death and birth rates.
#' In addition to evolving based on the SEIRD model, Populations of susceptible, exposed,
#' infectious and recovered individuals decline by the natural death rate, while the 
#' population of susceptible individuals increase by the birth rate. This model assumes that
#' individuals are born with no immunity.
#'
...
#' @describeIn SEIRD Set transmission parameters (beta, kappa, gamma and mu, lambda, nu)
#' of the SEIR model.
#'
#' If the transmission parameters provided to are not 1-dimensional an error is
#' thrown.
#'
#' @param object (SEIRD model)
#' @param value (list) list of values for beta, kappa, gamma, mu, lambda, nu, respectively.
#'
#' @return object of class SEIRD with transmission parameter values
#' assigned.
#' @aliases transmission_parameters<-,ANY,ANY-method
...
setMethod(
  ...
  function(object, times, solve_method = "lsoda") {
    ...
    parameters <- c(b = transmission_parameters(object)$beta,
                    ...
                    l = transmission_parameters(object)$lambda,
...
setMethod("R0", "SEIRD", function(model) {
  ...
  lambda <- model@transmission_parameters$lambda
