...
for (n in 1:length(countries)){
  ...
  #compute R0
  NU <- sum(rowSums(mat_urban) * pop_urban) / 2
  NY <- sum(rowSums(mat_rural) * pop_rural) / 2
  K <- ((1-frac_rural)* NU + frac_rural*NY)*C
  
  matF <- matrix(data = c(0, 0, b*S0U/(g+m)*(K + NU/(1-frac_rural)*(1-C)), b*S0U/(g+m)*K,
                          0, 0, b*S0Y/(g+m)*(K + NY/frac_rural*(1-C)),     b*S0Y/(g+m)*K,
                          0, 0,                 0,                              0,
                          0, 0,                 0,                              0), 
  ...
  store_R0[n] <- max(abs(eigen(matF %*% inv(matV))$values))
  store_R0_basic[n] <- b*((1-frac_rural)* NU + frac_rural*NY)/(g+m)
}

df <- data.frame (country  = c(countries,countries), model = c(rep("basic",length(countries)),rep("UvsR",length(countries))), R0 = c(store_R0_basic,store_R0))
