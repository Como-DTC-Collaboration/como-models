...
#' @rdname SEIRDAge_interventions-class
#' @export
...
#' single age group.
#'
#' @return Updated version of the age-structured SEIR model which.
#' includes 
#' @aliases initial_conditions<-,ANY,ANY-method
#' @rdname SEIRDAge_interventions-class
#' @export
...
setMethod(
  ...
  function(object, value) {
    ...
    if(length(beta_isolated) != 1 | length(kappa) != 1 | length(gamma) != 1){
