...
setMethod(
  ...
  function(object, value) {
    ...
    for (p in list("S0", "E0", "I0", "I0_isolated", "R0")) {
...
#' solve_method.
#'
#' \deqn{\frac{dS(t)}{dt} = - beta S(t) I(t) - beta_i S(t) I_i(t)}
#' \deqn{\frac{dE(t)}{dt} = beta S(t) I(t) + beta_i S(t) I_i(t) - kappa E(t)}
#' \deqn{\frac{dI(t)}{dt} = (1-eta) kappa E(t) - (gamma + mu) I(t)}
#' \deqn{\frac{dI(t)}{dt} = eta kappa E(t) - (gamma + mu) I_i(t)}
#' \deqn{\frac{dR(t)}{dt} = gamma I(t) + gamma I_i(t)}
#' \deqn{\frac{dC(t)}{dt} = beta S(t) I(t) + beta_i S(t) I_i(t)}
#' \deqn{\frac{dD(t)}{dt} = mu I(t) + mu I_i(t)}
#'
...
#' \deqn{\frac{dE(t)}{dt} = beta S(t) I(t) + beta_i S(t) I_i(t)- kappa E(t)}
#' \deqn{\frac{dI(t)}{dt} = (1 - eta) kappa E(t) - (gamma + mu) I(t)}
#' \deqn{\frac{dI(t)}{dt} = eta kappa E(t) - (gamma + mu) I(t)}
#' \deqn{\frac{dR(t)}{dt} = gamma I(t) + gamma I_i(t)}
#' \deqn{\frac{dC(t)}{dt} = beta S(t) I(t) + beta_i S(t) I_i(t)}
#' \deqn{\frac{dD(t)}{dt} = mu I(t) + mu I_i(t)}
#'
...
setMethod(
  ...
  function(object, times, solve_method = "lsoda") {
    ...
    state <- c(S = initial_conditions(object)$S0,
               ...
               I_isolated = initial_conditions(object)$I0_isolated,
    ...
    right_hand_side <- function(t, state, parameters) {
      with(
        as.list(c(state, parameters)), {
          ...
          d <- state[7]
          # rate of change
          ds <- - (b * s * i) - (b_isolated * s * i_isolated)
          de <- (b * s * i) + (b_isolated * s * i_isolated)- (k * e)
          di <- (k * e * (1-h) - (g + m) * i) 
          di_isolated = (k * e * h - (g + m) * i_isolated) 
          dr <- (g * i) + (g * i_isolated)
          dc <- b * s * i+ (b_isolated * s * i_isolated)
          d_death <- (m * i) + (m * i_isolated)
          # return the rate of change
          list(c(ds, de, di, di_isolated, dr, dc, d_death))
        })
    }
