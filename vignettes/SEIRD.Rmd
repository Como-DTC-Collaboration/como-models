...
since this shows the number of cases which have arisen inbetween these time  points. Deaths are also given by a difference, but, in this instance, between the cumulative number of deaths inbetween time points:
...
ggplot(changes, aes(x = time, y = value)) +
  geom_line(aes(color = compartment)) +
  scale_color_brewer(palette = "Dark2")
```

## Sensitivity analysis
In this section, we investigate the sensitivity of deaths to the transmission rate, $\beta$.

First, we run the model at various $\beta$ values and plot the deaths.
```{r, fig.height = 5, fig.width = 7}
...
result %>% 
  ...
  geom_line(aes(colour=as.factor(beta), group=as.factor(beta)))
```

Why do some epidemics die out whereas others don't? This is due to the $R_0$ parameter, the basic reproduction number, the expected number of cases directly generated by one case in a population where all individuals are susceptible to infection. If $R_0>1$, the epidemic grows; if $R_0<1$, transmission dies out over time. In an SEIRD model, the reproductive number is given by:

$$R_0 = \beta/\gamma.$$
We now calculate this quantity for the $\beta=0.1$ and $\beta=0.75$ cases.
```{r, fig.height = 5, fig.width = 7}
inits <- list(S0=0.99, E0=0, I0=0.01, R0=0)
...
This explains why the $\beta=0.1$ epidemic died out whereas the $\beta=0.75$ epidemic didn't!