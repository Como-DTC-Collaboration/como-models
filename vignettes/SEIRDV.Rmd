...
$\delta_V$, $\delta_R$ and $\delta_{VR}$. We take COVID parameters as in other vignettes where possible and estimated $\beta$ based on a scenarion without interventions.
```{r}
...
beta <- (gamma+mu)*R0_target
params <- list(beta, kappa, gamma, mu, nu=0.4, delta_V=0.1,
...
run_seirdv <- function(delta_V_val) {
  ...
  params <- list(beta, kappa, gamma, mu, nu=0.4, delta_V=delta_V_val,
                 delta_R=0.05, delta_VR = 0.02)
...
run_seirdv <- function(nu_val) {
  inits <- list(S0=0.99, E0=0, I0=0.01, R0=0, V0=0, VR0=0)
  params <- list(beta, kappa, gamma, mu, nu=nu_val, delta_V=0.1,
                 delta_R=0.05, delta_VR = 0.02)
  interv<- list(starts=c(17, 110, 150),
                stops=c(90, 139, 209),
                coverages=c(1, 1, 1))
