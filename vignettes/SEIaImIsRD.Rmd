...
We start by introducing the equations of the model and the computation of the basic reproduction number, $R_0$. We then show how the implementation of this model in the `comomodels` package can be used.
...
## Running simulations
In this section we walk through an example of how to use the `SEIaImIsRD` class, the implementation of the SEIaImIsRD model in the `comomodels` package. In doing so, we parameterise the model using transmission parameters meant to representat of the epidemiology of COVID-19.
...
First, we explore the impact of $R_0$ by varying this parameter and in each case counting the overall number of deaths. We plot $R_0$ versus the number of deaths and obtain a sigmoidal curve, which shows the greatest sensitivity around 1-1.5. Nonetheless, if we assume an uncertainty of $R_0=2.4\pm0.5$, we obtain a range from `r round(sensitivity_run(R0_target=1.9), 0)` to `r round(sensitivity_run(R0_target=2.9), 0)` deaths -- a substantial variation highlighting the central importance of knowing this quantity.
```{r, fig.align = "center", fig.show="hold",fig.height = 5, fig.width = 7}
