...
```
The maximum number of daily deaths predicted by this model is approximately `r round(max(output_deaths$value), 0)`, and overall, the number of deaths for such an unmitigated epidemic is predicted to be `r round(sum(output_deaths$value), 0)`-- a value comparable to the figure of 510,000 deaths predicted in report 9 [@ferguson2020report].
