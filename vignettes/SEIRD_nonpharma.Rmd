...
```{r}
params <- list(beta=0.5, beta_isolated=0.1, kappa=0.2, gamma=0.1, mu=0.048, heta1=0, heta2=1)
...
run_seird <- function(heta_val) {
  ...
  params <- list(beta=0.5, beta_isolated=0.1, kappa=0.9, gamma=0.5, mu=0.1, heta1=0, heta2=heta_val)
...
for(i in seq_along(heta_vals)) {
  ...
  temp <- run_seird(heta_temp) %>% mutate(heta2=heta_temp)
  if(i == 1)
    result <- temp %>% mutate(heta2=heta_temp)
  else
...
result %>% 
  ...
  ggplot(aes(x=time, y=value, fill = as.factor(heta2))) +
  geom_bar(stat = "identity", position = position_dodge()) +
