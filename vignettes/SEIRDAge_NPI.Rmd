...
vignette: >
  %\VignetteIndexEntry{The SEIRDAge_NPI model}
...
params <- list(beta_npi = 0.65, beta = 1,
...
and $beta_npi$. We now explore several NPI scenarios: **physical distancing**, **self-isolation of the infected groups**, **school closures**, **shielding of older population groups (ie, ≥70 years)**, and a combination of them (**lockdown**). 
...
run_SEIRDAge_NPI_single_intv <- function(beta_npi, contact_matrix_npi) {
  ...
  mu <- c(0.000016, 0.000016, 0.00007, 0.00007, 0.00031, 0.00031, 0.0026, 0.0026,
     0.0048, 0.0048, 0.006, 0.006, 0.019, 0.019, 0.043, 0.043)
  params <- list(beta_npi = beta_npi, beta = 1,
               kappa = 0.9, gamma = 0.5, mu = mu)
  model <- SEIRDAge_NPI(
...
model_si <- run_SEIRDAge_NPI_single_intv(0.65*1, list(c_combined_wider))
...
c_combined_wider_l <- c_home %>%
  ...
  combine_contact_matrices() %>% 
  combined_contact_matrices_wider()
model_l <- run_SEIRDAge_NPI_single_intv(0.65*1, list(c_combined_wider_l))
...
result[result$scenario!="baseline", "interv_starts"] <- 5
result[result$scenario!="baseline", "interv_stops"] <- 20
result_states[result_states$scenario!="baseline", "interv_starts"] <- 5
result_states[result_states$scenario!="baseline", "interv_stops"] <- 20
...
```{r, fig.height=5, fig.width=15}
run_SEIRDAge_NPI_mult_intv <- function(beta_npi, contact_matrix_npi, n_npi) {
  inits <- list(S0=pop_fraction*0.99,
                E0=rep(0, n_ages),
                I0=pop_fraction*0.01,
                R0=rep(0, n_ages),
  ...
  model <- SEIRDAge_NPI(
    ...
    interventions = list(list("starts"=5, "stops"=10, "coverages"=1),
                         list("starts"=15, "stops"=25, "coverages"=1)))
  times <- seq(0, 50, by = 1)
  out_df <- run(model, times)
  out_df
}
...
model_pd_pd <- run_SEIRDAge_NPI_mult_intv(c(1, 1), 
...
## self-isolation of the infected groups: beta_intervention = beta * 0.65
model_si_si <- run_SEIRDAge_NPI_mult_intv(c(0.65*1, 0.65*1),
...
model_pd_si <- run_SEIRDAge_NPI_mult_intv(c(1, 0.65*1), 
...
model_si_pd <- run_SEIRDAge_NPI_mult_intv(c(0.65*1, 1), 
...
# add intervention intervals
result[result$scenario!="baseline", "interv_starts"] <- c(5, 15)
result[result$scenario!="baseline", "interv_stops"] <- c(10, 25)
result_states[result_states$scenario!="baseline", "interv_starts"] <- c(5, 15)
result_states[result_states$scenario!="baseline", "interv_stops"] <- c(10, 25)
...
We can see different intervention results of the epidemic under different combinations of NPIs. (ADD MORE)
