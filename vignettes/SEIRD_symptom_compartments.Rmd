...
## Introduction
This document explains the basics of the SEIaImIsRD model. This model is an extension of the SEIRD model where instead of having one population of infected individuals there are three, each with their own level of symptoms. In this model, populations of asymptomatic, mildly symptomatic and severely symptomatic infectious individuals are included. The motivation for including these separate populations of infectious individuals comes from the fact that individuals with severe symptoms are much more likely to be detected as infected than asymptomatic individuals, leading case counts to underestimate the extent of the epidemic. Furthermore, the severeness of symptoms can correlate with the infectiousness of the individual, and their probablity of disease-related death: both of crucial importance for determining appropriate responses to epidemics.

Here, we introduce the `SEIaImIsRD` model included in the `comomodels` package, which lets the user simulate the SEIaImIsRD model, choosing the initial conditions and parameter values.

## The SEIaImIsRD Model
The SEIaImIsRD model describes the following population groups:

* **Susceptible** ($S$) - individuals who have not been exposed to the virus, whom can be infected by infectious individuals;

* **Exposed** ($E$) - individuals who have been exposed to the virus and have been infected, but are not yet infectious to others as they are within the incubation period;

* **Infectious** - individuals who can transmit the virus to others. In this model, we subdivide Infectious individuals into three sub-categories depending on their symptoms. Note that each compartment can be set with different transmission parameters.
...
* **Recovered** ($R$) - individuals who have recovered from the disease and obtained immunity to it. They cannot be infected again until their immunity wanes and they return to the susceptible population.


The SEIaImIsRD model consists of an ordinary differential equation (ODE) system of six ODEs, each describing the evolution of a different compartment over time:
...
* $\kappa$ - the rate at which exposed individuals become infectious. The incubation period thus lasts $1/\kappa$ days on average.
* $\eta_i$ - the fraction of exposed individuals moving into each of the different infected groups $i$. $\sum_i\eta_i=1$, $i\in\{a, m, s\}.$
...
In this section we walk through an example of how to use the `SEIaImIsRD`
class.
...
Next, we set the initial conditions. Because the populations are normalized, the populations must sum to one.
```{r}
S <- 0.99
E <- 0.01
asymptomatic <- 0
...
We then set the parameter values.
```{r}
...
Here, we define a helper function for plotting output dataframe of simulation result
```{r}
...
Lastly, we set time span and time step for the simulation outputs. Here we choose to output results at a daily resolution. Note, the numerical solvers used to integrate the ODEs (by default, the LSODA method) typically discretise the ODE at a finer resolution.
```{r}
t <- seq(0, 1000, by = 1)
```
...
We first assume that immunity does not last forever, so individuals who have recovered from the disease eventually return to the susceptible population. This is called waning immunity and is included in the model when $\omega>0$, as we assigned it above.
...
We can see that this affects the dynamics of the model quite a bit. The damped oscillations that we saw in the graph for the model with waning immunity have disappeared, we see fewer deaths, and the number of recovered individuals doesn't approach zero.
...
we see no change! This is because the basic reproduction number only indicates whether or not an epidemic can take off in a fully susceptible population. Once a sizeable proportion of the population have been infected, this is no longer the case.
...
```{r, fig.align = "center", fig.show="hold"}
model2 <- model
transmission_parameters(model2) <- list(beta = list(asymptomatic = 0.50, mild =  0.90, severe = 1.00),
                                        kappa = kappa, omega = omega,
                                        p_symptom = p_symptom, gamma = gamma,
                                        mu = mu)
