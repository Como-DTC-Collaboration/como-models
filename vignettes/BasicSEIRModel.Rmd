...
The S4 object class `SEIDR` lets the user
...
## The `SEIDR` class
In this section we will walk through an example of how to use the `SEIDR`
...
```{r}
out_df <- simulate_SEIDR(my_model, seq(0, 50, by = 0.1))
```
...
col <- c("Susceptibles" = "blue", "Exposed" = "green", "Infectious" = "yellow", "Recovered" = "red", "Cases" = "gray", "Deaths" = "black")
SEIRplot <- ggplot(out_df, aes(x = time)) +
    geom_line(aes(y = S, color = "Susceptibles"), size = 1.5) +
    geom_line(aes(y = E, color = "Exposed"), size = 1.5) +
    geom_line(aes(y = I, color = "Infectious"), size = 1.5) +
    geom_line(aes(y = R, color = "Recovered"), size = 1.5) +
    labs(x = "time", y = "fraction of the population",
         title = glue("SEIDR model")) +
...
inc_plot <- ggplot(out_df, aes(x = time)) +
    geom_line(aes(y = Incidences, color = "Cases"), size = 1.5) +
    geom_line(aes(y = Deaths, color = "Deaths"), size = 1.5) +
    labs(x = "time", y = "fraction of the population",
