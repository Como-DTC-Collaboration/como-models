...
## Introduction
A key aspect of modeling the spread of COVID-19 is the implementation of non-pharmaceutical interventions such as self isolating infected individuals, reducing physical contact through social distancing and shielding vulnerable groups. In this vignette, we implement the effect of a number of such interventions and examine their influence virus dynamics using an age structured SEIRD model.


# Underlying Model
For this analysis of interventions we utilise the age structured SEIRD model described in equations xx-xxx in our accompanying paper. The model describes how populations of susceptible, exposed, infectious, recovered and dead individuals evolve over time.
In this model, each age class, $i$, has its own system of five coupled differential equations governing movements between these compartments. We also allow for infections across age groups use a contact matrix, $C$, to allow infections to pass between age groups. The nature and role of the contact matrix is explored fully in the xxxxxx vignette. The governing eequations of the age structured model are therefore:
...
In order to implement different types on interventions we make ammendments to the governing equations above. These alterations are outlined below. 

# Isolating infected individuals

First, we implement the isolation of infected individuals by splitting the infected compartment for each age group ($I_i$) into those who decide to self isolate while they are infectious (denoted $I^{iso}_i$)
and those who decide not to isolate ($I^{noiso}_i$). We assume a constant fraction, denoted $\alpha$, of the total infected population choose to self isolate and that this fraction is constant across all age groups such that at any time such that

\begin{align}
I_i &= I^{iso}_{i} + I_{i}\\
I^{iso}_i &= \alpha I_{i} \\
I^{noiso}_i &= (1 - \alpha) I_{i}.
\end{align}

These two sub-compartments will spread the virus at different rates due to contrasting levels of social iteration with other groups. To allow for this, we amend the governing equations above such that the isolating and non-isolating infected compartments 
transmits the virus at rates $\beta^{iso}$ and $\beta^{noiso}$ respectively. The amended governing equations are then given by 

\begin{align}
\frac{\text{d}S_i}{\text{d}t} &= -\beta^{iso} S_i \Sigma_{j}C_{ij} I^{iso}_j - \beta^{noiso} S_i \Sigma_{j}C_{ij} I^{noiso}_j\\
\frac{\text{d}E_i}{\text{d}t} &= \beta^{iso} S_i \Sigma_{j}C_{ij} I^{iso}_j + \beta^{noiso} S_i \Sigma_{j}C_{ij} I^{noiso}_j -\kappa E_i\\
\frac{\text{d}I_i}{\text{d}t} &= \kappa E_i - (\gamma + \mu) I_i\\
\frac{\text{d}R_i}{\text{d}t} &= \gamma I_i - \alpha R_i\\
\frac{\text{d}D_i}{\text{d}t} &= \mu I_i
\end{align}
...
Now we create a model that includes this age-structured mixing. Since the contact matrices have 16 age classes, we instantiate a model with this number of classes. We also give it transmission parameters, populate the age classes with initial conditions and provide it with our overall contact matrix.

We use more demographics loaded from the population data file provided by the como model and, for this example, initialise the model
