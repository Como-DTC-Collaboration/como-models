---
title: "Optimisation with SEIRD model"
...
## Introduction
This document shows an application of the SEIRD model class within the como-models package. The SEIRD model can be used to gain insights from real COVID-19 data, such as extracting transmission parameters values. After fitting model to data, it can be used to make prediction of the pandemic. However, due to limited information, presence of errors and other problems, different sets of optimised parameters can fit the model to the data, achieving same objective value. This creates an issue of identifiability in parameters. Therefore, we have to be caution when intepreting optimisation results.

In this vignette, we want to show how optimisation is done with our package and the identifiabilty of parameters in the SEIRD model.

## Optimisation on real London COVID-19 data
First, we would optimise the data on London COVID-19 data. The time period of data we used is before any intervention is implemented, lockdown in this case. Every intervention alters the parameters of the SEIRD model. Since we are fitting only one set of parameters, constant throughout the whole period, we chose to use London COVID-19 data before lockdown.

The population size for London is defined to be $8.9 \times 10^6$, the estimated population of London extracted from [find source for data on trustforlondon.org.uk/data/population-over-time/] 
...
The graph shows the incidences and deaths in London, starting from 1st January 2020 to 22 March 2020. 23 March 2020 is the start of the lockdown.

For the optimisation, we want to maximise its Poisson log-likelihood function, which is defined as below. It takes the average of both log-likelihood values of incidences and deaths. 
...
## Optimisation
Due to the heuristic approach of Nelder-Mead method, the optimisation of the SEIRD model is repeated 100 times.
...
print(b)
```
The graph shows the 100 fitted models on the actual incidences and deaths. It can be observed from the graph that different runs of optimisation returns different results. 
...
The box plot shows a range of log-likelihood values achieved from different runs of optimisation. This confirms that some runs of optimisation perform better than others. This could happen when the optimising algorithm reaches a local optimum. The Nelder-Mead algorithm used here is a local optimisation algorithm. 

## Sort paramters according to error value and choose the top few
We choose the top 20 optimisation with highest log-likelihood values. By repeating the optimisations and choosing the few best ones, it is essentially a pseudo-global optimisation. 
...
The graph shows the 20 optimisations that achieve highest log-likelihood value.
...
In this graph, we can see that even when the optimisation fits the real data well during early stages of the pandemic, different outcomes are predicted. Given the estimated London population, the maximum difference in peak values of all projections is more than $1.5 \times 10^5$.

## Box plot of estimated variables and log-likelihood value
...
The box plot shows the range of optimised parameters of optimisations with top 20 highest log-likelihood values. Despite being the pseudo-global optimum, the transmission parameters, except $\mu$, display a distribution of optimised values, with large standard deviation. This shows that different sets of transmission parameters can achieve similar log-likelihood values. Given the data, the model cannot identify the parameters uniquely.   
...
To address the identifiability problem observed in optimisations of real data, an identifiability analysis is done using profile likelihood. The profile likelihood is the maximum likelihood value obtained by fixing a parameter of interest and optimising the remaining parameters. Here, the profile likelihood is constructed using synthetic data. 

## Simulate data

The data is simulated with values that provide sufficient time points before the pandemic peaks. This would replicate what is obtained from real data, such as the London data shown above.
...
The synthetic data is then drawn from Poisson distribution with $\lambda$ as the incidences and deaths of model simuated value, as shown in the equations below. $C(t)$ and $D(t)$ are number of incidences and deaths at time $t$ respectively. while $C^{*}(t)$ and $D^{*}(t)$ are number of incidences and deaths with Poisson noise. 
...
```
The graph shows the synthetic data simulated for full pandemic.
...
Since most of the pandemic data without intervention we can get ends before the peak. The first 70 times points of the synthetic data is chosen, which gives roughly 40 times points since the first case, similar to the London data. 

## Create error function

Poisson log-likelihood function is defined for optimisation of synthetic data. The profile likelihood is constructed for transmission parameters, while the initial conditions are fixed at values used to simulate the data. 
...
In some cases, identifiability issues arise when there are more parameters describing the model than the information can infer. In other words, parameters cannot be uniquely identified with limited data. When the number of parameters increase, parameters might become unidentifiable. Here, we first create a profile likelihood for two of the four transmission parameters in the SEIRD model. 
...
While working through the optimisations for different fixed values of parameter of interest, the previously optimised parameters were used as initial guesses for the next round of optimisation. Due to the local optimiser we used, the range of parameters for the profile likelihood starts from the actual parameter value to the minimum and then from the actual parameter value to the maximum.  
...
The graph shows the log-likelihood value obtained by fixing the parameter of interest and optimising the remaining one. The curve with single peak indicates that both $\beta$ and $\gamma$ are identifiable when the SEIRD model is simplified to have only two transmission parameters. However, there is a bias between the parameter value with maximum log-likelihood value and the actual parameter value used to simulate the data. The real parameter value is indicated by the vertical line. The bias is due to the simulation of synthetic data with Poisson noise. 
...
All curves from optimised models are plotted with the synthetic data in the graph. 

# Profile likelihood of all transmission parameters

The same procedure is repeated with four transmission parameters instead of two, which includes $\beta$, $\kappa$, $\gamma$ and $\mu$.
...
It is clear that $\beta$, $\kappa$ and $\gamma$ are not identifiable as they achieve similar log-likelihood values for different values of these parameters. For example, fixing $\beta = 1.2$ would give a fit of same log-likelihood value as fixing $\beta = 0.9$. Therefore, it could not be uniquely identified which $\beta$ value gives the best fit. In this case, only $\mu$ is identifiable as it is the death rate, change in other parameters cannot compensate $\mu$.
...
As expected, all optimisations perform equally well.

# Profile likelihood with more information

When more information is provided to the optimisation, the identifiability issue could be improved. Here, we show an example by constructing the profile likelihood using synthetic data with double time points. 
...
With more data points, $\mu$ is still identifiable, observed by a curve with single peak in the profile likelihood. $\kappa$ is marginally identifiable, with small gradients in the curve. On the other hand, $\beta$ and $\gamma$ are identifiable up to a certain range. The log-likelihood value for $\beta$ decreases when $\beta$ is smaller than 0.6.
...
grid.arrange(case_beta, case_kappa, case_gamma, case_mu, nrow = 2, ncol = 2)
```
These graphs show that some fitting do not perform as well as others, as expected from the profile likelihood.
