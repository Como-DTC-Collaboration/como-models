...
## Introduction
This document shows an application of the SEIRD model class within the como-models package. The SEIRD model can be used to gain insights from real COVID-19 data, such as incubation rate and death rate of the virus. By fitting the SEIRD model to data, we can make predictions of the pandemic. However, due to limited information and presence of errors, different sets of parameters can fit the model to the data with same objective value. This creates an issue of identifiability in parameters. Therefore, we have to be caution when intepreting optimisation results.
...
First, we would optimise the data on London COVID-19 data. The time period of data we used is set to be the period before any interventions is implemented, lockdown in this case. Every interventions alter the parameters of the SEIRD model. Since we are fitting only one set of parameters, constant throughout the whole period, so we chose to use the London COVID-19 data up to the implementation of lockdown.

The population size for London is defined to be $9 \times 10^6$. It is the estimated population of London in 2020 extracted from [1] 
...
print(b)
```
The graph shows the 100 fitted models on the actual incidences and deaths. It can be observed from the graph that different runs of optimisation returns different results. Some of the fits have their peaks in the first 10 days of the pandemic.
...
We choose the top 20 optimisation with highest log-likelihood values.
...
In this graph, we can see that even when the optimisation fits the real data well during early stages of the pandemic, different outcomes are predicted. Given the estimated London population, the maximum difference in peak values of all projections is more than $2 \times 10^5$.
...
To address the identifiability problem observed in optimisations of real data, an identifiability analysis is done using profile likelihood. The profile likelihood is a series of maximum likelihood value obtained by fixing a parameter of interest to a range of values and optimising the remaining parameters. Here, the profile likelihood is constructed using synthetic data. 

## Simulate data

The data is simulated with values that provide sufficient time points before the peak of the pandemic. This would replicate London COVID-19 data shown above.
...
The simulated data is then drawn from Poisson distribution with $\lambda$ as the incidences and deaths of model simuated value, as shown in the equations below. $C(t)$ and $D(t)$ are number of incidences and deaths at time $t$ respectively. while $C^{*}(t)$ and $D^{*}(t)$ are number of incidences and deaths with Poisson noise. 
...
The first 70 times points of the simulated data is chosen, which gives roughly 40 times points since the first case, similar to the London COVID-19 data. 

## Create error function

Poisson log-likelihood function is defined for optimisation of SEIRD model to simulated data. The profile likelihood is constructed for transmission parameters, while the initial conditions are fixed at values used to simulate the data. 
...
The graph shows the log-likelihood value obtained by fixing the parameter of interest and optimising the remaining tranmission parameters. The actual parameter value is indicated by the vertical line. The curves with single peak each indicate that both $\beta$ and $\gamma$ are identifiable when the SEIRD model is simplified to have only two transmission parameters. However, there is a bias between the parameter value with maximum log-likelihood and the actual parameter value used to simulate the data. The bias is due to the Poisson noise added to the simulated data. 
...
All curves from optimised models are plotted with the simulated data in the graph. 

# Profile likelihood of all transmission parameters

The same procedure is repeated with all the transmission parameters instead of two, which includes $\beta$, $\kappa$, $\gamma$ and $\mu$.
...
It is clear that $\beta$, $\kappa$ and $\gamma$ are not identifiable as they achieve similar log-likelihood values for different fixed values. For example, fixing $\beta = 1.2$ would give a fit of the same log-likelihood value as fixing $\beta = 0.9$. Therefore, it could not be uniquely identified which $\beta$ value gives the best fit. In this case, only $\mu$ is identifiable as it is the death rate, change in other parameters cannot compensate $\mu$. $\mu$ uniquely describes the number of deaths.
...
With more data points, $\mu$ is still identifiable, observed by a curve with single peak in the profile likelihood. $\kappa$ is marginally identifiable, with gradual decrease from the peak of the curve. On the other hand, $\beta$ and $\gamma$ are identifiable up to a certain range. The log-likelihood value for $\beta$ decreases when $\beta$ is smaller than 0.6, while the log-likelihood value for $\gamma$ decreases when $\gamma$ is smaller than 0.38.
