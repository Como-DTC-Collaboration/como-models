...
ErrorFn <- function(parameters, model=SEIRD(), inc_numbers=0, death_numbers=0,
  ...
  data_wide$Incidence <- data_wide$Incidence * population_size
  data_wide$Deaths <- data_wide$Deaths * population_size
  
  incidence_err <- sqrt(sum((inc_numbers - data_wide$Incidence[-1])^2))
  death_err <- sqrt(sum((death_numbers - data_wide$Deaths[-1])^2))
...
kappa_range <- seq(0.6, 1.2, by = 0.02)
gamma_range <- seq(0.3, 0.8, by = 0.02)
mu_range <- seq(0.05, 0.15, by = 0.005)
...
for (param_name in profile_parameters){
  ...
  for (i in 1:nrow(temp)){
    model <- SEIRD()
    transmission_parameters(model) <- list(beta=temp[i,4], kappa=temp[i,5], gamma=temp[i,6], mu=temp[i,7])
...
total_cases <- transform(total_cases, fixed_value=as.character(fixed_value))
