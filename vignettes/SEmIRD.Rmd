...
$$\frac{\text{d}R}{\text{d}t} = \gamma I,$$
$$\frac{\text{d}D}{\text{d}t} = \mu I.$$
Thus a newly exposed individual (entering $E_1$) only becomes infectious (entering $I$) after they have passed through a host of intermediate states: $E_1\rightarrow E_2 \rightarrow E_3 \rightarrow E_4 \rightarrow E_5$. In the stochastic analogue of this ODE system, the average time taken to go from $E_1\rightarrow I$ is given by a $\text{Erlang}(5,\kappa)$ distribution, which has a mean of $5/\kappa$. Thus to match the same mean as the exponential distribution we need to multiply $\kappa$ by 5.
