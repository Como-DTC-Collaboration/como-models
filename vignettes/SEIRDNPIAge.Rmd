...
Next, we need to set the transmission parameter values $\beta$, $\kappa$, $\gamma$ and $\mu$. We again obtain COVID parameter values for $\kappa$, $\gamma$ and $\mu$from the function `covid_transmission_parameters`, the derivation of which has been set out in the vignettes of the `SEIRDAge` model for an age-structured model.
