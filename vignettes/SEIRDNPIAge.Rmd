...
Consistent with the [SEIRDAge model](https://github.com/Como-DTC-Collaboration/como-models/blob/main/vignettes/SEIRD_age_structured.Rmd), in the `SEIRDNPIAge` model describes how populations of susceptible, exposed, infectious, recovered and dead individuals evolve over time. Intermixing between age groups is accounted for via social contact matrices $C$, where the elements $C_{ij}$ indicate the expected number of contacts an individual of age class $i$ experiences with age class $j$ per day. In this package, we provide country-level estimates of contact matrices [@prem2017projecting], which are broken down according to whether contacts occurred at home, work, school and "other" places.

To provide an example, we first visualise the contact matrices for India.
...
infectious and are protected from reinfection. Here, we assume that there is no waning natural immunity, so once individuals are recovered from infection they never become susceptible again. Births and natural deaths (i.e. those unrelated to COVID-19 infection) are not considered in this model. 
...
$$\frac{\text{d}E_i}{\text{d}t} = \beta_i(t) S_i \Sigma_{j}C_{ij}(t) I_j-\kappa E_i,$$
...
The key parameters of the `SEIRDNPIAge` model are: $\beta_i(t)$ and $C_{ij}(t)$ representing time-dependent rates of infection and contact matrix elements, respectively. This model allows for discontinuous changes in either the rate parameter and / or the contact matrix over time in one or a number of intervention blocks.
...
We first load the country-level demographics from UN and set the number of age classes to be the same in the contact matrices, which is 16 in this instance.
...
In model initiance, the parameters `n_ages` and `n_npi` represent the number of age classes and the number of NPI intervals, respectively; `contact_matrix` is a single matrix that represents the original contact matrix when no intervention is applied, whereas `contact_matrix_npi` defines a list of matrices corresponding to the contact matrices of each intervention period. The last parameter `age_ranges`
...
At this point, $\kappa$ is defined; $\gamma_i$ and $\mu_i$ can be derived; $\beta_i(t)$ needs to be set. In the setter function `transmission_parameters` for `SEIRDNPIAge`, the parameter `beta` defines the transmission rate for each age group
...
run_SEIRDNPIAge_single_intv <- function(beta_npi_frac, contact_matrix_npi) {
...
model_base <- run_SEIRDNPIAge_single_intv(1, list(c_combined_wider))
...
model_pd <- run_SEIRDNPIAge_single_intv(1, list(c_combined_wider_pd))
...
model_si <- run_SEIRDNPIAge_single_intv(0.65, list(c_combined_wider))
...
model_sc <- run_SEIRDNPIAge_single_intv(1, list(c_combined_wider_sc))
...
c_combined_wider_so <- c_home %>%
  ...
  combine_contact_matrices() %>% 
  combined_contact_matrices_wider()
model_so <- run_SEIRDNPIAge_single_intv(1, list(c_combined_wider_so))
...
c_combined_wider_l <- c_home %>%
  ...
  combine_contact_matrices() %>% 
  combined_contact_matrices_wider()
model_l <- run_SEIRDNPIAge_single_intv(0.65, list(c_combined_wider_l))
...
```{r, fig.height=5, fig.width=15}
run_SEIRDNPIAge_mult_intv <- function(beta_npi_frac, contact_matrix_npi, n_npi) {
  inits <- list(S0=pop_fraction*0.999,
                E0=rep(0, n_ages),
                I0=pop_fraction*0.001,
                R0=rep(0, n_ages),
...
model_pd_pd <- run_SEIRDNPIAge_mult_intv(c(1, 1), list(c_combined_wider_pd, c_combined_wider_pd), 2)
...
## self-isolation of the infected groups: beta_intervention = beta * 0.65
model_si_si <- run_SEIRDNPIAge_mult_intv(c(0.65, 0.65), list(c_combined_wider, c_combined_wider), 2)
...
model_pd_si <- run_SEIRDNPIAge_mult_intv(c(1, 0.65), list(c_combined_wider_pd, c_combined_wider), 2)
...
model_si_pd <- run_SEIRDNPIAge_mult_intv(c(0.65, 1), list(c_combined_wider, c_combined_wider_pd), 2)
