...
Which is reflected in the incidences and deaths.
```{r, fig.width=8, fig.height=5}
changes %>%
  ggplot(aes(x=time, y=value, colour=age_range)) +
  geom_line() +
...
We now simulate the same model but with different distributions of initial infections to show the sensitivity of the model to initial conditions. First, we set all initial infections to 0.3% of the total population and place this 0.1% uniformly across age groups between 15 and 30. Second, we place all infections (also 0.3% of the total population) uniformly across age groups 55-70. 
```{r}
...
Now we simulate both models and plot the infections over time. We can see that for the model initialised with infections among 15-30 years olds, the peak of infections appears earlier and dies away quicker than the model initialised with infections in 55-70 year olds. Despite this difference, the magnitude of the infection peaks in both bases are comparable as well as the distribution of infections for each age group. 
...
res %>%
  filter(compartment=="I") %>% 
  ggplot(aes(x=time, y=value, colour=age_range)) +
  ggtitle("Infections (55-70 years Initialised)") +
  xlab("Time") + ylab("I Fraction") +
  theme(plot.title = element_text(size=20)) +
  geom_line() +
  facet_wrap(~compartment)
