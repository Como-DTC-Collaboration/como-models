...
c_combined_wider <- c_combined %>% 
  ...
  select(-age_infectee) %>% 
  as.matrix()
model <- comomodels::SEIRAge(n_age_categories = n_ages,
                 contact_matrix = c_combined_wider,
                 age_ranges = as.list(age_names))
transmission_parameters(model) <- list(b=1, k=0.5, g=0.5)
...
```{r}
model_1530 <- comomodels::SEIRAge(n_age_categories = n_ages,
                 contact_matrix = c_combined_wider,
                 age_ranges = as.list(age_names))
model_5070 <- comomodels::SEIRAge(n_age_categories = n_ages,
                 contact_matrix = c_combined_wider,
                 age_ranges = as.list(age_names))

transmission_parameters(model_1530) <- list(b=1, k=0.5, g=0.5)
transmission_parameters(model_5070) <- list(b=1, k=0.5, g=0.5)
