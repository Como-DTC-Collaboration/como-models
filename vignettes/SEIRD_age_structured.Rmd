...
For the transmission rate, we assume an arbitrary value $\beta=1$ which produces epidemic dynamics (that is, $R_0>1$). 
```{r}
kappa <- 1 / 5.5
eta <- 1 / 2
beta <- 0.35
...
```{r}
death_plus_recovery <- mu_vals + gamma_vals
c_times_s <- sweep(c_combined_wider, 1, pop_fraction, "*")
G <- t(t(0.35 * c_times_s) / death_plus_recovery)
eigen(G)$values[1]
R0(model, pop_fraction)
```


We next simulate the model and visualise all the dynamics of each compartment.
```{r, fig.width=10, fig.height=5}
res <- run(model, times=seq(0, 500, by = 0.1))
states = res$states
changes = res$changes
states %>%
