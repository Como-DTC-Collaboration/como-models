...
```{r}
params <- list(beta=0.5, beta_isolated=0.1, kappa=0.2, gamma=0.1, mu=0.048, heta=1)
...
# run model across different heta values
heta_vals <- c(0, 0.25, 0.5, 0.75, 1)
for(i in seq_along(heta_vals)) {
  heta_temp <- heta_vals[i]
  temp <- run_seird(heta_temp) %>% mutate(heta=heta_temp)
  if(i == 1)
    result <- temp %>% mutate(heta=heta_temp)
  else
...
result %>% 
  ...
  ggplot(aes(x=time, y=value, fill = as.factor(heta))) +
  geom_bar(stat = "identity", position = position_dodge()) +
