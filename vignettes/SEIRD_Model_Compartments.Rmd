...
where $\beta$ denotes the rate at which an infected individual infects susceptible individuals,
...
where $\eta_{E->I_a}$, $\eta_{E->I_m}$, $\eta_{E->I_s}$ denote the fraction of exposed individuals that develop asymptomatic, mild and severe symptoms, respectively (in the code the fraction is denoted by 'p_symptom').
...
infectious, and the rate at which an infectious individual infects a 
...
Steady states of the system described above are computed by setting the right-hand sides of the ODEs equal to zero. However, here, we solve the ODEs over time instead of just solving for the steady states. The
...
First, define $x\in (x_1, x_2,...,x_n)^T$ to be the number of individuals in each compartment, where the first $m < n$ compartments contain exposed or infected individuals. In the **SEIaImIsRD** model, these correspond to compartments $E$, $I_a$, $I_m$ and $I_s$. 
