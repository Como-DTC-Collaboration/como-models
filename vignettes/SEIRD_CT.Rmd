...
We first suppose that self isolation upon becoming symptomatic is the only NPI in place, and that 50% of people follow this (i.e. $\pi=0$ and $\phi=0.5$).
...
big_df %>%  
  ...
  xlab("time (days)")
```
Comparing the R0 values under these policies
```{r}
calculate_R0
```
